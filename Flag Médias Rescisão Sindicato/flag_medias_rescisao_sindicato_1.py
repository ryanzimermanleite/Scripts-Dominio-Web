...
def abre_tela_sindicato_empregados():
    ...
    time.sleep(3)
    while not _find_img('rescisao_aba.png', conf=0.9):
        time.sleep(1)
...
def consulta_rescisao(andamentos, texto_extraido):
    status1 = ''
    status3 = ''
    time.sleep(1)
    if _find_img('considerar_ok.png', conf=1):
        status1 = 'OK'
    if _find_img('calcular_ok.png', conf=1):
        status3 = 'OK'
    # Função para verificar e clicar nas imagens
    time.sleep(1)

    if _find_img('erro.png', conf=1):

        status2 = 'Não foi possivel Marcar'
    elif status1 == 'OK' and status3 == 'OK':

        status2 = 'OK'
    else:

        while not _find_img(f'considerar_ok.png', conf=1):
            _click_img(f'considerar.png', conf=0.9)
            time.sleep(5)
            _click_img('rescisao.png', conf=0.9)
            time.sleep(1)
        time.sleep(1)
    ...
    p.click(481, 165)
    time.sleep(5)
    while not _find_img('rescisao_aba_2.png', conf=0.9):
        time.sleep(1)
        _click_img('medias.png', conf=0.9)
        time.sleep(2)
        _click_position_img('ferias.png', '-', pixels_x=35, conf=0.9)
        time.sleep(1)
    time.sleep(1)
...
def run(controle):
    ...
    while True:
        ...
        resultado = consulta_rescisao(andamentos, texto_extraido)
...
if __name__ == '__main__':
    andamentos = 'Flag Médias Rescisão Sindicato'
