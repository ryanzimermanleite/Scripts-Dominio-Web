...
def run(window, values):
    ...
    if not all([input_excel_cliente, codigo]):
        ...
        try:
            principal(input_excel_cliente, codigo, tipo_pagar, tipo_receber, numero_banco, check, window)
        except:
            pass



def principal(input_excel_cliente, codigo, tipo_pagar, tipo_receber, numero_banco, check, window):
...
def separa_notas_encontradas_e_nao_encontradas(codigo, nome_excel):
    ...
    # Converter a planilha encontrada em um dicionário para acesso rápido
    encontrado_dict = data_encontrado.set_index('Número da Nota').to_dict('index')
    ...
    for index, row in data_nao_encontrado.iterrows():
        ...
        if nota in encontrado_dict:
            ...
            row['CPF/CNPJ do fornecedor'] = encontrado_dict[nota]['CPF/CNPJ do fornecedor']
            row['Data de Vencimento da parcela'] = encontrado_dict[nota]['Data de Vencimento da parcela']
    ...
    if linhas_encontradas:
        ...
        df_notas_encontradas.to_csv(path_base + f'{codigo} - Notas Encontradas.csv', index=False, sep=';',
                                    encoding='ISO-8859-1')
    ...
    if linhas_nao_encontradas:
        ...
        df_notas_nao_encontradas.to_csv(path_base + f'{codigo} - Notas Pendentes.csv', index=False,
