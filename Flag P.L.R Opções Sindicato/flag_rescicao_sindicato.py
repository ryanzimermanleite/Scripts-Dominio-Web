...
from _comum.comum_comum import _escreve_relatorio_csv, _escreve_header_csv, _get_host_name, _time_execution_monitor_db
...
def abre_tela_sindicato_empregados():
    ...
    time.sleep(3)
    while not _find_img('opcoes_config.png', conf=0.9):
        time.sleep(1)
        _click_img('aba_plr.png', conf=0.9)
        time.sleep(3)
    time.sleep(1)
    _click_img('opcoes.png', conf=0.9)

def consulta_plr(andamentos, texto_extraido):
    # Função para verificar e clicar nas imagens
    def verificar_e_clicar(imagem, conf=0.9):
        if not _find_img(f'{imagem}_ok.png', conf=1):
            _click_img(f'{imagem}.png', conf=conf)
            time.sleep(5)
            _click_img('opcoes.png', conf=conf)
            time.sleep(1)
    while not _find_img('padrao.png', conf=1):
        for i in range(1, 10):
            verificar_e_clicar(str(i))

    _click_img('3pontos.png', conf=0.9)
    while not _find_img('selecao_afastamento.png', conf=0.9):
        time.sleep(0.5)
    _click_img('titulo_afastamento.png', conf=0.9)
    time.sleep(0.5)
    while not _find_img('padrao2.png', conf=1):
        if not _find_img('acidente_trabalho.png', conf=1):
            _click_img(f'acidente_trabalho2.png', conf=0.9)
            time.sleep(1)
            _click_img('titulo_afastamento.png', conf=0.9)

        if not _find_img('licenca_maternidade.png', conf=1):
            _click_img(f'licenca_maternidade2.png', conf=0.9)
            time.sleep(1)#
            _click_img('titulo_afastamento.png', conf=0.9)
...
def run(controle):
    ...
    while True:
        ...
        resultado = consulta_plr(andamentos, texto_extraido)
...
if __name__ == '__main__':
    andamentos = 'Flag PLR Rescição Sindicato'
